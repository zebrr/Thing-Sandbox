...
| `config.toml` | Параметры приложения | таймауты LLM, retry, дефолтная модель |
| `simulation.json` | Параметры симуляции | `current_tick`, `status`, `memory_cells` |
...
```toml
[llm]
model = "gpt-4o-mini"           # дефолтная модель
timeout = 60                     # таймаут запроса (секунды)
max_retries = 3                  # количество retry при ошибках
retry_delay = 5                  # задержка между retry (секунды)

[simulation]
memory_cells = 5                 # количество ячеек детальной памяти (K)
```

### Параметры симуляции

Хранятся в `simulation.json` каждой симуляции:

| Параметр | Описание |
|----------|----------|
| `memory_cells` | Количество ячеек детальной памяти (K) |
