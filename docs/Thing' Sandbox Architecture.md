...
**JSON-схемы** (`*.schema.json`) — это **документация контракта** для человеческого чтения. Они описывают структуру данных, но код их не читает в рантайме.

**Валидация в рантайме** идёт через **Pydantic модели**, определённые в коде:
- Данные симуляции: `utils/storage.py` (Character, Location, Simulation)
- Ответы LLM: в каждой фазе (IntentionResponse в `phase1.py`, и т.д.)

**Почему так:**
- OpenAI SDK принимает Pydantic модели напрямую для structured output
- Pydantic автоматически генерирует JSON Schema под капотом
- Избегаем дублирования логики валидации

**Правило:** при изменении структуры данных — обновляй и Pydantic модель в коде, и JSON-схему в `src/schemas/` для документации.
...
## 11. Формат логирования

Все модули используют стандартный модуль `logging` с единым форматом вывода для консоли.

### Формат строки

```
YYYY.MM.DD HH:MM:SS | LEVEL   | 🏷️ module: message
├── 19 символов ────┤ 7 симв ─┤ переменная ──────────────
```

**Пример вывода:**

```
2025.06.05 14:32:07 | INFO    | ⚙️ config: Loaded from /path/to/config.toml
2025.06.05 14:32:07 | INFO    | 🎬 runner: Starting tick 42 for demo-sim
2025.06.05 14:32:08 | INFO    | 🎭 phase1: Processing 2 characters
2025.06.05 14:32:09 | INFO    | 🎭 phase1: Ogilvy → "Approach the cylinder cautiously"
2025.06.05 14:32:09 | WARNING | 🎭 phase1: Henderson fallback to idle (invalid location)
2025.06.05 14:32:10 | INFO    | 🤖 llm: Batch complete (2 requests, 450 tokens)
2025.06.05 14:32:10 | WARNING | 🤖 llm: Rate limit hit, retry 1/3 in 2s
2025.06.05 14:32:15 | INFO    | 💾 storage: Saved demo-sim (tick 42)
2025.06.05 14:32:15 | INFO    | 🎬 runner: Tick 42 complete (8.2s)
```

### Эмодзи по модулям

| Эмодзи | Модуль | Область |
|--------|--------|---------|
| ⚙️ | config | Конфигурация |
| 🎬 | runner | Оркестрация тиков |
| 🎭 | phase1 | Намерения |
| ⚖️ | phase2a | Арбитраж |
| 📖 | phase2b | Нарратив |
...
|---------|--------------------|
| DEBUG | Детали операций (содержимое запросов, chain ops) |
| INFO | Ключевые события (tick start/end, phase complete, save) |
| WARNING | Recoverable issues (fallback, retry, missing optional data) |
| ERROR | Перед выбросом исключения |

### Правила

- **Только logging, без print()** — весь user-facing вывод через logging с настроенным console handler
- **Logger создаётся** как `logger = logging.getLogger(__name__)`
- **Сообщения без точки в конце** — `"Loaded config"`, не `"Loaded config."`
- **Контекст в сообщении** — включать entity id, file path, метрики где релевантно

---

## 12. Поток такта: технические детали
