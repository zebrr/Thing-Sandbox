...
**Базовые шаблоны** находятся в `src/prompts/` — минимальные и универсальные, с плейсхолдерами вместо сеттинга.

**Кастомные промпты** конкретной симуляции находятся в `simulations/{sim-id}/prompts/`. Кастомный промпт может добавлять:
- Сеттинг мира
...
Это позволяет переопределять любой промпт для конкретной симуляции, не дублируя все остальные.
...
### 2.5 Language Inference

Промпты указывают: "Respond in the language of the simulation content".

LLM определяет язык по контенту в user prompt:
- Описания персонажей (identity.description)
- Описания локаций
- Записи памяти

Если симуляция на русском — весь контент на русском, LLM отвечает на русском. Если на английском — на английском.

**Смешанный контент:** если часть контента на одном языке, часть на другом — поведение непредсказуемо. Рекомендуется единый язык для всей симуляции.
...
**Примечание:** Phase 2b не получает `tick` — нарратор описывает события сцены, не привязываясь к номеру тика. Если нужна нумерация в нарративе, добавьте tick в кастомный промпт симуляции.
...
#### Примечание: triggers в нескольких фазах

Triggers передаются в Phase 1, 2a и 2b — это не избыточность, а разное использование:
- **Phase 1** — персонаж знает свои паттерны поведения
- **Phase 2a** — арбитр учитывает триггеры при разрешении конфликтов
- **Phase 2b** — нарратор понимает мотивацию для лучшего описания
...
- Перемещать персонажей в несуществующие локации (использовать только id из `valid_location_ids`)

**Формат memory_entry:**
- От первого лица персонажа, субъективно
- 3-5 предложений — одно ключевое событие или осознание
- Детали и рутина могут опускаться

**Изменение локации:**
- `moment` — обновлять ВСЕГДА (что происходит прямо сейчас)
- `description` — обновлять РЕДКО, только при значимых изменениях (разрушение, строительство, катастрофа). Вернуть `null` если без изменений.

**Пустые локации (без персонажей):**
- Арбитр всё равно обрабатывает локацию
- Обновляет только `location.moment` — мир живёт
- Не генерирует characters в output (пустой объект)
- Примеры: костёр догорает, дождь усиливается, солнце садится
...
1. LOCATION
   ...
   - state (moment)
   - valid_location_ids — список всех существующих локаций (защита от галлюцинаций при перемещении)
...
- Потоки данных: `docs/Thing' Sandbox Data Flow.md`